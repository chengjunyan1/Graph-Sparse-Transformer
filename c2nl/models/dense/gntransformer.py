...
from c2nl.models.diffused_transformer import TransformerEncoder# GAEncoder
...
class Encoder(nn.Module):
    ...
    def forward(self, inputs, mask, seq_length=None):
        layer_outputs, _ = self.transformer(inputs, mask)
        return layer_outputs[-1][:,:seq_length,:] if seq_length!=None else layer_outputs[-1]
...
class GNTransformer(nn.Module):
    ...
    def _run_forward_ml(self,
        ...
        code_rep = self.embedder(astok,
                                 ...
                                 pos_enc=wpos,
                                 mode='encoder')
        memory_bank = self.encoder(code_rep, mask, seq_length)  # B x seq_len x h

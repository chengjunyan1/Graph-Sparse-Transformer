...
class GALayer(nn.Module):
    def __init__(self, embed_size, d_k, d_v, num_heads, rate=0.2, diffuse=0, alpha=0.15):
        ...
        self.gat=GATConv(in_feats=embed_size, out_feats=self.F_, d_k=d_k, d_v=d_v,
                    num_heads=num_heads, attn_drop=rate, diffuse=diffuse, alpha=alpha)
...
class GABlock(nn.Module):
    def __init__(self, embed_size, d_k, d_v, num_heads, num_layers, hus, rate=0.2, diffuse=0, alpha=0.15):
        super(GABlock, self).__init__()
        self.num_layers=num_layers
        self.GALayers=nn.ModuleList([GALayer(embed_size, d_k, d_v, num_heads, rate, diffuse, alpha)
...
class GAEncoder(EncoderBase):
    def __init__(self, 
        ...
        self.GABlocks=nn.ModuleList([GABlock(d_model, d_k, d_v, heads,
            num_layers, d_ff, dropout, diffuse, alpha) for i in range(num_blocks)])

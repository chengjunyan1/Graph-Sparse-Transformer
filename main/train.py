...
def add_train_args(parser):
    ...
    files.add_argument('--model_name', type=str, default='GTA',
                       help='Unique model identifier (.mdl, .txt, .checkpoint)')
    files.add_argument('--data_dir', type=str, default='./data/',
                       help='Directory of training/validation data')
    files.add_argument('--train_src', nargs='+', type=str, default=['train/code.original_subtoken'],
                       help='Preprocessed train source file')
    files.add_argument('--train_tgt', nargs='+', type=str, default=['train/javadoc.original'],
                       help='Preprocessed train target file')
    files.add_argument('--train_ast', nargs='+', type=str, default=['train/syntax.adddata'],
                       help='Preprocessed train syntax file')
    files.add_argument('--dev_src', nargs='+', type=str, default=['dev/code.original_subtoken'],
                       help='Preprocessed dev source file')
    files.add_argument('--dev_tgt', nargs='+', type=str, default=['dev/javadoc.original'],
                       help='Preprocessed dev target file')
    files.add_argument('--dev_ast', nargs='+', type=str, default=['dev/syntax.adddata'],
...
def init_from_scratch(args, train_exs, dev_exs):
    ...
    # Initialize model
    model = GSTransformerAST(config.get_model_args(args), src_dict, tgt_dict)
...
def main(args):
    ...
    if args.only_test:
        if args.pretrained:
            model = GSTransformerAST.load(args.pretrained)
        else:
            if not os.path.isfile(args.model_file):
                raise IOError('No such file: %s' % args.model_file)
            model = GSTransformerAST.load(args.model_file)
    else:
        if args.checkpoint and os.path.isfile(args.model_file + '.checkpoint'):
            # Just resume training, no modifications.
            logger.info('Found a checkpoint...')
            checkpoint_file = args.model_file + '.checkpoint'
            model, start_epoch = GSTransformerAST.load_checkpoint(checkpoint_file, args.cuda)
        else:
            # Training starts fresh. But the model state is either pretrained or
            # newly (randomly) initialized.
            if args.pretrained:
                logger.info('Using pretrained model...')
                model = GSTransformerAST.load(args.pretrained, args)
            else:
